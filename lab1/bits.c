...
int evenBits(void)
{
  int evenPos;
  evenPos = 0x55;
  evenPos = evenPos | (evenPos << 8);
  evenPos = evenPos | (evenPos << 16);
  return evenPos;
}
// 3
/*
...
{
  int signX, signY, subtracted, subResultSign, opSign;

  signX = x >> 31;
  signY = y >> 31;

  subtracted = x + (~y + 1);
  subResultSign = subtracted >> 31;

  opSign = signX ^ signY;

  return (opSign & !signX) | (!(opSign | subResultSign) & !!subtracted);
}
// 7
/*
...
int twosComp2SignMag(int x)
{
  ...
  mag = (x & ~onlySign) | ((~x + 1) & onlySign);

  return (onlySign << 31) + mag;
}
// 10
/*
